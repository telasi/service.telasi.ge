...
class Ext::Gis::Transformator
  ...
  def sync_current_status
    ...
    if tr.blank? or tr.enabled == 1
      ...
      log = Ext::Gis::Log.where(objectid: self.objectid, table_name: 'mv_tr_pnt', :gis_status.ne => 1).desc(:_id).first
      self.on = false
  ...
  def self.sync_current_status_and_notify(phones_key = 1)
    ...
    if total > 0
      ...
      text << 'თელასის ქსელი სრული დატვირთვით მუშაობს: ყველა აბონენტს მიეწოდება დენი.'
      text_ru << 'Сеть Теласи полностью нагруженна: все потребители получают эл.энергию.'
    end
    if Magti::SEND
