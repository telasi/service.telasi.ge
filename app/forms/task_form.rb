...
module TaskForm
  ...
  def self.task_table(tasks, cust = nil)
    ...
    category = TextField.new(name: 'category')
    title_size = ComplexField.new(label: 'დავალების შინაარსი', fields: [SIZE.clone, category, TITLE.clone], url: TITLE.url)
  ...
  def self.search(operators, regions, params)
    ...
    form.col1 << SelectField.new(name: 'category', label: 'კატეგორია', collection: Call::Category.asc(:order_by), empty: '(ყველა კატეგორია)')
    form.col1 << SelectField.new(name: 'status', label: 'სტატუსი', collection: Call::Status.asc(:order_by), empty: '(ყველა სტატუსი)')
    form.col2 << SelectField.new(name: 'user', label: 'ოპერატორი', collection: operators, empty: '(ყველა ოპერატორი)')
    form.col2 << SelectField.new(name: 'region', label: 'რეგიონი', collection: regions, empty: '(ყველა რეგიონი)')
