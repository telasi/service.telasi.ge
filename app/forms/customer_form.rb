...
module CustomerForm
  ...
  def self.customer_form(cust, opts = {})
    # col1
    pre = ComplexField.new(fields: [NumberField.new(name: 'pre_payment', after: 'GEL'), DateField.new(name: 'pre_payment_date', formatter: '%d-%b%Y %H:%M:%S', not_local: true)], label: 'დაუდ.გადახდა')
